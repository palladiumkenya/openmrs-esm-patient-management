...
import { UnScheduleAppointmentResponse } from '../hooks/useUnscheduledAppointments';
...
export function DownloadAppointmentAsExcel(
  ...
  const appointmentsJSON = appointments?.map((appointments) => {
    return {
      ...
      Date: formatDate(new Date(appointments.dateTime), { year: true }),
...
export function DownloadUnscheduleAppointments(
  appointments: Array<UnScheduleAppointmentResponse>,
